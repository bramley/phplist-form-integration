...
$phplist_strings = array(
	'field1' => '<input type="text" name="'. $o['php_list_txt_id'] . '" id="'. $o['php_list_txt_id'] .'" size="'. $o['php_list_txt_size'] .'" maxlength="'. $o['php_list_txt_max'] .'" value="' . paranoid($_POST[$o['php_list_txt_id']], array('-', ' ')) . '" /> '. (strlen($o['php_list_txt_req'])>0 ? '(required)':  '') . '<br/>',
	'email' => '<input type="text" name="email" id="email" size="'. $o['php_list_email_size'] .'" maxlength="'. $o['php_list_email_max'] .'" value="'. paranoid($_POST['email'], array('@', '.', '_', '-')) .'" /> (' . __('required', 'phplist') . ')' .'<br/>',
	'error' => '');
...
function phplist_check_input()
{
    ...
    if (strlen($required)>0 && $txt_show) {
			if(empty($_POST[$txt_id]))  {
				...
				$phplist_strings['field1'] = '<input class="requiredOutline" type="text" name="'. $txt_id .'" id="' . $txt_id .'" size="' . $txt_size . '" maxlength="'. $txt_max . '" value="' .$optionvalue . '"  /> (' . __('required', 'phplist') . ')';
			}
	}
    if(!is_email($email))
    {
	    $ok = false; $reason = 'Invalid Email';
	    $phplist_strings['email'] = '<input class="requiredOutline" type="text" name="email" id="email" size="'. $email_size .'" maxlength="'. $email_max .'" value="' . $email . '"  /> (' . __('required', 'phplist') . ')';
	}
